...
Ini
[Service]
WorkingDirectory=/home/<user>/cardano-node/relay
ExecStart=/usr/local/bin/cardano-node run \
  --config /home/<user>/cardano-node/relay/config/config.json \
  --topology /home/<user>/cardano-node/relay/config/topology.json \
  --database-path /home/<user>/cardano-node/relay/db \
  --socket-path /home/<user>/cardano-node/relay/node.socket \
  --host-addr 0.0.0.0 \
  --port 3001
LimitNOFILE=131072
Restart=always
...
Ini
[Service]
WorkingDirectory=/home/<user>/cardano-node/bp
ExecStart=/usr/local/bin/cardano-node run \
  --config /home/<user>/cardano-node/bp/config.json \
  --topology /home/<user>/cardano-node/bp/config/topology.json \
  --database-path /home/<user>/cardano-node/bp/db \
  --socket-path /home/<user>/cardano-node/bp/node.socket \
  --host-addr 127.0.0.1 \
  --port 3002 \
  --shelley-kes-key /home/<user>/cardano-node/bp/kes.skey \
  --shelley-vrf-key /home/<user>/cardano-node/bp/vrf.skey \
  --shelley-operational-certificate /home/<user>/cardano-node/bp/node.cert
LimitNOFILE=131072
Restart=always
...
cardano-cli latest stake-pool registration-certificate \
    ...
    --out-file pool.cert
6. 保守運用の鉄則
DB保護: シャットダウン時は systemctl stop を実行し、データのFlush（数秒〜数十秒）を待つ。
Pledge維持: 宣言したPledge額（4500 ADA）を下回らないよう残高を管理。
ディスク管理: TraceMempool: false 設定と SystemMaxUse=500M でログ肥大化を防止。
回線安定化: SPO運用には有線LAN接続を推奨。
